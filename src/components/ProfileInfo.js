...
export default function ProfileInfo() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const msgValue = document.getElementById("designation").value;
    await db
      .collection("profiles")
  ...
  useEffect(() => {
    user.updateEmail(emailRef.current.value);
    db.collection("profiles")
      .doc(auth.currentUser.uid)
      .get("designation")
      .then((response) => {
        if (response.exists)
          if (response.data().hasOwnProperty("designation")) {
            const abdc = response.data();
            document.getElementById("designation").value = abdc.designation;
          } else {
            console.log("Designation is not yet assigned");
          }
      });
  });
  ...
  return (
    <>
      <div className="profileInfoScreen">
        ...
        <h1 className="topText">My Profile</h1>
        <div className="signupCard profileCard">
          <div className="backBtn" onClick={goBack}>
            <ArrowBackIcon />
            Back
          </div>
          <form className="signupForm" onSubmit={handleSubmit}>
            <label className="formLabelSignup">Full Name</label>
            <input
              ref={nameRef}
            ...
            <input
              ...
              type="email"
            />
            <label className="formLabelSignup">Photo URL</label>
            <input
              ref={photoRef}
              ...
              type="url"
            />
            <label className="formLabelSignup">Designation</label>
            <input
              id="designation"
              className="formInputSignup"
              type="text"
